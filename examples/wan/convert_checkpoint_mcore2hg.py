...
parser.add_argument("--save_path", type=str, required=True, help="Path to save hg checkpoints to")
...
for i in range(num_layers):
    ...
    src_qkv_weight = mcore_dict["decoder.layers." + str(i) + ".self_attention.linear_qkv.weight"]
    trans_qkv = rearrange(src_qkv_weight, "(N R D) H -> (R N D) H", R=3, N=40, D=128, H=5120)
    ...
    src_qkv_bias = mcore_dict["decoder.layers." + str(i) + ".self_attention.linear_qkv.bias"]
    ...
    cross_q_weight = mcore_dict["decoder.layers." + str(i) + ".cross_attn.linear_q.weight"]
    cross_q_weight = rearrange(cross_q_weight, "(N R D) H -> (R N D) H", R=1, N=40, D=128, H=5120)
    cross_q_bias = mcore_dict["decoder.layers." + str(i) + ".cross_attn.linear_q.bias"]
    cross_q_bias = rearrange(cross_q_bias, "(N R D H) -> (R N D H)", R=1, N=40, D=128, H=1)
    ...
    kv_weight = mcore_dict["decoder.layers." + str(i) + ".cross_attn.linear_kv.weight"]
    ...
    # cross_attention o
    cross_o_weight = mcore_dict["decoder.layers." + str(i) + ".cross_attn.linear_proj.weight"]
    cross_o_weight = rearrange(cross_o_weight, "(N R D) H ->(R N D) H", R=1, N=40, D=128, H=5120)
    new_state_dict["blocks." + str(i) + ".cross_attn.o.weight"] = cross_o_weight

    cross_o_bias = mcore_dict["decoder.layers." + str(i) + ".cross_attn.linear_proj.bias"]
    cross_o_bias = rearrange(cross_o_bias, "(N R D H) ->(R N D H)", R=1, N=40, D=128, H=1)
