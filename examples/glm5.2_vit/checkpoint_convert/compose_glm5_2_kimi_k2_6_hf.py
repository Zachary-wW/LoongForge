...
def compose(glm_hf: Path, kimi_hf: Path, output: Path, materialize: bool = False) -> None:
    ...
    try:
        composed_map = _link_weight_map(glm_map, glm_hf, temp_path, "glm", materialize=materialize)
        composed_map.update(
            _link_weight_map(
                kimi_map,
                kimi_hf,
