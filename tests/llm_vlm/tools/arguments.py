...
def _print_available_models(args, print_func):
    ...
    available = ConfigManager.list_all_available_models(
        config_dir, extra_configs_dirs=all_dirs[1:] if len(all_dirs) > 1 else None
    )
...
def parse_args():
    """Parse all arguments."""
    parser = argparse.ArgumentParser(description="loongforge tools", allow_abbrev=False)
    config_dir = "configs"
    ...
    all_available_models = ConfigManager.get_all_models(
        config_dir, extra_configs_dirs=[optional_config_dir], recursive_extra=True
    )
