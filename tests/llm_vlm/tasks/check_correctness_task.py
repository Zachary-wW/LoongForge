...
class CorrectnessCheckTask(BaseTask):
    def __init__(
        ...
        super().__init__(model_description, task_description, model, model_configer, args)
                    ...
                    if (
                        ...
                        if "RUNNABLE_FLAG" not in self.model["scenarios"][index][scenario_name][
                            ...
                            self.start_loongforge_reverse_convert_ckpt(
                                index, step1_5_name, scenario_name, training_type_name
                            )
                            step1_5_scenario_lock_file = os.path.join(
                                self.model["model_lock_file_path"],
                                scenario_name,
                                step1_5_name,
                                self.master_addr,
