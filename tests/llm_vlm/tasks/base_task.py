...
class BaseTask(object):
    ...
    def validate_metrics(self, model_name, training_type=None):
        ...
        # elapsed_time_ms
        if (
            hasattr(self.metric, "elapsed_time_match")
            and self.metric.elapsed_time_match
            and "elapsed_time_ms" in baseline_data[0]
        ):
        ...
        if (
            hasattr(self.metric, "mem_allocated_avg_MB")
        ...
        if (
            hasattr(self.metric, "mem_max_allocated_avg_MB")
    ...
    def collect_metrics(self, training_log_file, strict=True):
        ...
        if strict:
            assert len(self.metric.lm_loss_list) != 0, (
