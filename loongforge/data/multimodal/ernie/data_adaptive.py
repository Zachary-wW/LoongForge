...
class Adaptive:
    ...
    def set_video_pixels(self, video_min_pixels=None, video_max_pixels=None, msg=""):
        ...
        if video_min_pixels is not None:
            assert isinstance(video_min_pixels, int) and video_min_pixels >= 0, (
                "video_min_pixels must be positive int"
            )
            logger.info(f"{msg} Adaptive set video_min_pixels = {video_min_pixels}")
            self.video_min_pixels = video_min_pixels
        if video_max_pixels is not None:
            assert isinstance(video_max_pixels, int) and video_max_pixels > 0, (
    ...
    def process(self, sample: dict, sample_grouped_info):
        ...
        for frames in grouped_frames:
            if image_info[frames[0]]["image_type"] == "video":
                assert len(frames) % self.temporal_conv_size == 0, (
    ...
    def get_images_token_num(
        ...
        for frames in grouped_frames:
            if image_info[frames[0]]["image_type"] == "video":
                ...
                tmp = [
                    self.get_num_of_tokens_for_img_one(image_info[f], min_pixels, max_pixels)
                    for f in frames
                ]
                ret.append(sum(tmp))
            else:
