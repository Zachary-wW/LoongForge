...
class HFChatTemplate(ChatTemplate):
    ...
    def _chunk_tokenize_mask(
        ...
        boundaries = sorted(
            {0, len(text)} | {boundary for span in generation_ranges for boundary in span}
        )
        chunk_ids: List[int] = []
        chunk_mask: List[int] = []
    ...
    def _truncate_to_assistant_boundary(
        ...
        if len(input_ids) != len(assistant_masks):
            raise ValueError(
                f"assistant mask length must match input_ids length, got {len(assistant_masks)} vs {len(input_ids)}"  # noqa: E501
            )
    ...
    def _build_labels_from_assistant_mask(
        ...
        labels = [
            token_id if mask else ignore_index for token_id, mask in zip(input_ids, loss_mask)
        ]
...
_register_chat_template(
    ...
    format_user=StringFormatter(
        slots=[
            "<|im_user|>user<|im_middle|>{{content}}<|im_end|><|im_assistant|>assistant<|im_middle|><think></think>"
        ]
    ),
...
_register_chat_template(
    ...
    format_user=StringFormatter(
        slots=[
            "<|im_user|>user<|im_middle|>{{content}}<|im_end|><|im_assistant|>assistant<|im_middle|><think>"
        ]
    ),
    format_system=StringFormatter(slots=["<|im_system|>system<|im_middle|>{{content}}<|im_end|>"]),
    format_assistant=StringFormatter(slots=["{{content}}<|im_end|>"]),
