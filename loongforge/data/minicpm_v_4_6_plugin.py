...
class MiniCPMV46Plugin(MMPlugin):
    ...
    def _target_sizes_to_grid_thw(target_sizes):
        ...
        if target_sizes.dim() != 2 or target_sizes.shape[-1] != 2:
            raise ValueError(
                f"Expected MiniCPM target_sizes with shape [n, 2], got {tuple(target_sizes.shape)}."
            )
        ones = torch.ones(
    ...
    def _image_placeholder(self, token_count: int, image_index: int, *, use_image_id: bool) -> str:
        placeholder = (
            f"{self.image_start_token}{self.image_token * token_count}{self.image_end_token}"
        )
        if use_image_id:
            placeholder = (
                f"{self.image_id_start_token}{image_index}{self.image_id_end_token}" + placeholder
            )
        return placeholder
