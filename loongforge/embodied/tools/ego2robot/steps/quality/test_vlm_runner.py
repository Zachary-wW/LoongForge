...
class _FakeContainer:
    def __init__(self, frame_count: int, fps: float):
        ...
        self.fps = fps
        self.streams = SimpleNamespace(video=[SimpleNamespace(average_rate=fps)])
    ...
    def decode(self, _stream):
        return (_FakeFrame(index, self.fps) for index in range(self.frame_count))


class VideoSamplingTest(unittest.TestCase):
    def test_uniform_indices_cover_full_sequence(self):
        self.assertEqual(vlm_runner._uniform_indices(100, 5), [0, 25, 50, 74, 99])
        self.assertEqual(vlm_runner._uniform_indices(3, 5), [0, 1, 2])

    def test_sample_video_caps_and_uniformly_covers_timeline(self):
        fake_av = SimpleNamespace(open=lambda _path: _FakeContainer(frame_count=100, fps=10.0))
        with mock.patch.dict(sys.modules, {"av": fake_av}):
            images = vlm_runner._sample_video(
                Path("unused.mp4"), fps=10.0, max_frames=5, max_edge=448, jpeg_quality=100
            )
    ...
    def test_none_uses_hard_default_cap(self):
        ...
        with mock.patch.dict(sys.modules, {"av": fake_av}):
            images = vlm_runner._sample_video(
                Path("unused.mp4"), fps=10.0, max_frames=None, max_edge=448, jpeg_quality=85
            )
