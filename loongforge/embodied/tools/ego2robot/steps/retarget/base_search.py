...
def estimate_scene_support_surface(scene_depth, head_seq, K, targets_p_world, reach):
    ...
    camera_down = np.mean([_quat_to_mat(pose[3:7])[:, 1] for pose in head_seq[frame_ids]], axis=0)
    ...
    down_hi = float(np.percentile(target_down, 90.0)) + max(0.50, 1.20 * float(reach))
    z_lo, z_hi = sorted((down_sign * down_lo, down_sign * down_hi))

    world_chunks = []
    depth_min, depth_max = SUPPORT_SURFACE_DEPTH_RANGE
    for frame_id in frame_ids:
        z_cam = depth[frame_id][::SUPPORT_SURFACE_PIXEL_STRIDE, ::SUPPORT_SURFACE_PIXEL_STRIDE]
        valid = np.isfinite(z_cam) & (z_cam >= depth_min) & (z_cam <= depth_max)
        if not np.any(valid):
            continue
    ...
    min_vertical_component = 1.0 / np.sqrt(1.0 + SUPPORT_SURFACE_MAX_SLOPE**2)
    ...
    for _ in range(SUPPORT_SURFACE_RANSAC_ITERATIONS):
        ...
        xy_eigenvalues = np.linalg.eigvalsh(centered_xy.T @ centered_xy / len(centered_xy))
    ...
    xy_eigenvalues = np.linalg.eigvalsh(centered_xy.T @ centered_xy / max(len(centered_xy), 1))
    if rmse > SUPPORT_SURFACE_MAX_RMSE or xy_eigenvalues[0] < 0.0025:
        return None

    if len(support_points) > SUPPORT_SURFACE_MAX_POINTS:
        keep_ids = np.linspace(0, len(support_points) - 1, SUPPORT_SURFACE_MAX_POINTS, dtype=int)
...
def _base_visual_min_z(model, base_body):
    ...
    base_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, str(base_body))
    if base_id < 0:
    ...
    for geom_id in range(model.ngeom):
        ...
        if int(model.geom_type[geom_id]) != int(mujoco.mjtGeom.mjGEOM_MESH):
            min_z = min(min_z, float(data.geom_xpos[geom_id, 2] - model.geom_rbound[geom_id]))
            continue
...
def _snap_base_to_support(base_pos, support_surface):
    ...
    plane_z = coefficients[0] * base_pos[0] + coefficients[1] * base_pos[1] + coefficients[2]
    base_up_z = float(support_surface.get("base_up_z", 1.0))
    base_pos[2] = plane_z - base_up_z * float(support_surface.get("base_min_z", 0.0))
    points_xy = np.asarray(support_surface["points_xy"], dtype=float)
    if len(points_xy):
        local_gap = float(np.sqrt(np.min(np.sum((points_xy - base_pos[:2]) ** 2, axis=1))))
    else:
        local_gap = float("inf")
    return base_pos, local_gap


def _print_progress(label, done, total, started, last_print, force=False, width=28):
...
def base_frame(fwd_flat, up_axis=None):
    ...
    z = np.array([0.0, 0.0, 1.0]) if up_axis is None else np.asarray(up_axis, dtype=float)
...
def select_base_keyframes(targets_p_world, targets_R_world, max_keyframes=BASE_MAX_KEYFRAMES):
...
def _screen_orientation_candidates(R_nominal):
    ...
    return [(R, euler) for R, euler in _base_orientation_candidates(R_nominal) if euler in wanted]


def _candidate_score(record):
    return record["feasibility_rate"] - 5.0 * abs(record["reach_ratio"] - BASE_REACH_TARGET)
...
def search_base_pose_original(
    ...
    idx = (
        ...
        else np.linspace(0, n_frames - 1, max_keyframes).astype(int)
    )
    ...
    best_record = None
    best_score = None
    dz_offsets = (0.0,) if support_surface is not None else DLS_HAND_ANCHOR_DZ
    for back in DLS_HAND_ANCHOR_BACK:
        for lat in DLS_HAND_ANCHOR_LAT:
            for dz in dz_offsets:
                base_pos = anchor - fwd * back + lateral * (lat * sign) + np.array([0.0, 0.0, dz])
...
def search_base_pose(
    ...
    base_forward_anchor=None,
    base_forward_offsets=None,
    ...
    if base_orientation_mode not in {"free", "upright"}:
        raise ValueError(f"unknown base orientation mode: {base_orientation_mode}")
    ...
    fixed_base_orientation = base_orientation_mode == "upright" or (
    ...
    if not position_records:
        ...
        fallback_pos = anchor - 0.3 * fwd * reach + sign * 0.4 * lateral * reach + 0.15 * up * reach
        if support_surface is not None:
            fallback_pos, _ = _snap_base_to_support(fallback_pos, support_surface)
        position_records = [
            {
    ...
    if search_mode == "slow":
        print(
            f"    base_search coarse grid: {len(position_records)} translations x "
    ...
    for position in position_records:
        ...
        for base_R, euler in all_orientations:
            ...
            for frame_idx in eval_idx:
                ...
                errors_pos.append(err_pos)
                errors_rot.append(err_rot)
                successes += int(err_pos < BASE_FEAS_POS_TOL and err_rot < BASE_FEAS_ROT_TOL)
            record = {
    ...
    if search_mode == "slow":
        ...
        for refine_done, coarse in enumerate(coarse_records, 1):
            ...
            refine_last_print = _print_progress(
                "base_search refine", refine_done, refine_total, refine_started, refine_last_print
            )
        if refine_total:
            _print_progress(
...
def _evaluate_base_pair_collisions(
    ...
    dual = build_dual_model(
        left_choice["base_pos"],
        left_choice["base_quat"],
        right_choice["base_pos"],
        right_choice["base_quat"],
        fovy_deg=60.0,
        spec=spec,
    )
    ...
    for t in frame_idx:
        ...
        if use_robust_ik:
            left_q, left_ok, left_ep, left_er = solve_arm_ik_robust(
                ...
                left_base_R.T @ left_targets_R[t],
                q_warm=warm_left,
                tol_pos=BASE_FEAS_POS_TOL,
                tol_rot=BASE_FEAS_ROT_TOL,
                mink_context=mink_context,
            ...
            right_q, right_ok, right_ep, right_er = solve_arm_ik_robust(
                ...
                right_base_R.T @ right_targets_R[t],
                q_warm=warm_right,
                tol_pos=BASE_FEAS_POS_TOL,
                tol_rot=BASE_FEAS_ROT_TOL,
                mink_context=mink_context,
        ...
        data.qpos[:] = qpos
        mujoco.mj_forward(dual, data)
        contacts, frame_penetration, frame_min_distance = _cross_arm_collision_metrics(dual, data)
        if contacts:
            collision_frames += 1
...
def select_joint_base_pair(
    ...
    left_targets_p=None,
    left_targets_R=None,
    right_targets_p=None,
    right_targets_R=None,
    left_keyframes=None,
    right_keyframes=None,
    spec=None,
    mink_context=None,
    mink_position_context=None,
    ik_solver=solve_arm_ik,
    use_robust_ik=False,
    enable_collision_screen=True,
    base_alignment_axis=None,
):
            ...
            pairs.append(
                {
                    ...
                    "left": left,
                    "right": right,
                    "base_height_delta": abs(float(left["base_pos"][2] - right["base_pos"][2])),
                    "base_depth_delta": (
                        0.0
                        if alignment_axis is None
                        else abs(float((left["base_pos"] - right["base_pos"]) @ alignment_axis))
                    ),
                }
            )
    ...
    if spec is not None and spec.coplanar_bases:
        ...
        if coplanar:
            ...
            pairs = [p for p in pairs if p["base_height_delta"] <= min_delta + 1e-9]
    ...
    if spec is not None and spec.aligned_base_depths:
        ...
        if aligned:
            ...
            pairs = [p for p in pairs if p["base_depth_delta"] <= min_delta + 1e-9]

    collision_enabled = (
        spec is not None
        and single is not None
        and arm_qadr is not None
        and arm_vadr is not None
        and arm_ids is not None
        and ee_ref is not None
        and left_targets_p is not None
        and left_targets_R is not None
        and right_targets_p is not None
        and right_targets_R is not None
        and left_keyframes is not None
        and right_keyframes is not None
        and enable_collision_screen
    )
    ...
    print(
        f"    dual-arm collision screen: {len(pairs)} pairs, "
