...
def build_dreamzero_sampler(
    ...
    * ``distributed`` -> torchdata ``StatefulDistributedSampler``.
    ...
    sampler_num_workers = (
        num_workers if data_cfg.sampler_num_workers is None else int(data_cfg.sampler_num_workers)
    )
    ...
    if sampler_type == "global_batch_shard":
        ...
        if rank == 0:
            logger.info(
                "[dreamzero-data] using global batch-shard sampler seed=%s micro_batch=%s replicas=%s dataset_len=%s",  # noqa: E501
                sampler_seed,
                batch_size,
                replicas,
                len(train_dataset),
    ...
    if sampler_type == "distributed":
        ...
        return None, True
    raise ValueError(
        f"Unknown DreamZero sampler_type={sampler_type!r}; expected distributed, sharded, or global_batch_shard"  # noqa: E501
    )
