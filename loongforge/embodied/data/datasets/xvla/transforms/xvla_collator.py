    ...
    def __call__(self, examples: List[Dict[str, Any]]) -> XVLABatch:
        ...
        if all("image_input" in ex for ex in examples):
            ...
            batch["image_input"] = torch.stack(
                [
                    ex["image_input"]
                    if isinstance(ex["image_input"], torch.Tensor)
            ...
            batch["image_mask"] = torch.stack(
                [
                    ex["image_mask"]
                    if isinstance(ex["image_mask"], torch.Tensor)
            ...
            for ex in examples:
                ...
                batch_images.append([XVLAEncodeImageTransform._to_pil(ex[k]) for k in image_keys])
        ...
        batch["action"] = torch.stack(
            [
                ex["action"]
                if isinstance(ex["action"], torch.Tensor)
