...
def _all_gather_variable_dim(
    ...
    pieces = [
        _slice_dim(tensor, dim, 0, size) for tensor, size in zip(gathered, sizes, strict=True)
    ]
...
def _check_4d_all_to_all_input(
    ...
    if x.dim() != 4:
        raise ValueError(
            f"DreamZero Ulysses expects a 4D [B, L, H, D] tensor, got {tuple(x.shape)}"
        )
...
def _all_to_all_4d_list(
    ...
    *,
    ...
    input_list = [
        chunk.contiguous()
        for chunk in torch.split(x, _split_sizes(x.shape[scatter_dim], world_size), dim=scatter_dim)
    ]
    if scatter_dim == 2 and gather_dim == 1:
        ...
        output_list = [
            torch.empty(local_shape, dtype=x.dtype, device=x.device) for _ in range(world_size)
        ]
    else:
...
class DreamZeroSplitGather(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx: Any, x: torch.Tensor, group: dist.ProcessGroup | None, dim: int
    ) -> torch.Tensor:
