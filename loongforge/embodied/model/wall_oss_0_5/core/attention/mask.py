...
def update_joint_attention_mask_2d(
    ...
    causal_mask = torch.tril(torch.ones((seq_len, seq_len), dtype=torch.bfloat16, device=moe_token_types.device))
    ...
    if not causal_action_attention_mask:  # If causal action attention mask is disabled, set the whole moe1 region to 1
        attention_mask = torch.where(moe1_mask, torch.ones_like(attention_mask), attention_mask)

    if positional_masks is not None and "ar_predict_token_positions" in positional_masks:
        ar_predict_token_positions = positional_masks["ar_predict_token_positions"]
        moe1_mask = (moe_token_types[:, :, None]) & (ar_predict_token_positions[:, None, :])
        attention_mask = torch.where(moe1_mask, torch.zeros_like(attention_mask), attention_mask)

    if positional_masks is not None and "valid_flow_action_positions" in positional_masks:
        # true in moe_token_types but false in valid_flow_action_positions
        nonvalid_flow_action_positions = moe_token_types & ~positional_masks["valid_flow_action_positions"]
        attention_mask = torch.where(
    ...
    if positional_masks is not None and "ar_action_mask" in positional_masks:
        ...
        if positional_masks.get("ar_visible", True):
            ...
            attention_mask = torch.where(flow_ar_mask, torch.ones_like(attention_mask), attention_mask)
        else:
