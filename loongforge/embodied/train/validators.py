...
def _validate_fsdp_ignored_frozen_args(training_args):
    ...
    if training_args.distributed_strategy != "fsdp":
        raise ValueError("--fsdp-ignore-frozen-module-classes requires --distributed-strategy fsdp")
    ...
    if training_args.fsdp_ignored_frozen_param_dtype is not None:
        ...
        ignored_dtype = dtype_aliases.get(training_args.fsdp_ignored_frozen_param_dtype)
        compute_dtype = dtype_aliases.get(training_args.dtype)
        if ignored_dtype is None or compute_dtype is None or ignored_dtype != compute_dtype:
            raise ValueError(
...
def validate(training_args, model_cfg, data_cfg):
    ...
    if training_args.save_interval < 0:
        raise ValueError(f"--save-interval must be >= 0, got {training_args.save_interval}")
    ...
    if training_args.hsdp_shard_size is not None and training_args.hsdp_shard_size <= 0:
    ...
    if training_args.use_pytorch_profiler and training_args.use_nsys_profiler:
        raise ValueError("--use-pytorch-profiler and --use-nsys-profiler are mutually exclusive.")
    ...
    if training_args.lora_alpha <= 0:
        raise ValueError(f"--lora-alpha must be positive, got {training_args.lora_alpha}")
    if not 0.0 <= training_args.lora_dropout < 1.0:
        raise ValueError(f"--lora-dropout must be in [0, 1), got {training_args.lora_dropout}")
    if training_args.use_lora:
        if training_args.init_on_meta:
            raise ValueError("--use-lora does not currently support --init-on-meta")
        if training_args.save_interval == 0:
            logger.warning("--save-interval=0 disables both checkpoints and LoRA adapter saving.")
    ...
    if training_args.fp8:
        ...
        if training_args.fp8_backend == "te" and getattr(model_cfg, "compile_model", False):
            raise ValueError(
        ...
        if training_args.fp8_backend == "torchao":
            ...
            raise ValueError("TorchAO-specific FP8 options require --fp8-backend=torchao.")
