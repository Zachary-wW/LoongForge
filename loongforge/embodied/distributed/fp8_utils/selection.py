...
def resolve_linear_selection(
    ...
    linear_keys = [
        module_key for module_key, module in model.named_modules() if module_key and type(module) is nn.Linear
    ]

    matched_patterns: set[str] = set()
    selected_keys = [module_key for module_key in linear_keys if _select(module_patterns, module_key, matched_patterns)]
    unmatched_patterns = [pattern for pattern in module_patterns if pattern not in matched_patterns]
    if unmatched_patterns:
        raise ValueError("fp8 module patterns matched no nn.Linear: " + ", ".join(unmatched_patterns))

    matched_skip_patterns: set[str] = set()
    skipped_keys = {
        module_key for module_key in selected_keys if _select(skip_patterns, module_key, matched_skip_patterns)
    }
    unmatched_skip_patterns = [pattern for pattern in skip_patterns if pattern not in matched_skip_patterns]
    if unmatched_skip_patterns:
        raise ValueError("fp8 skip modules were not selected: " + ", ".join(unmatched_skip_patterns))
...
def _select(
    ...
    candidate_keys = [".".join(segments[:depth]) for depth in range(1, len(segments) + 1)]
    selected = False
    for pattern in patterns:
        if any(is_module_path_matching_pattern(pattern, candidate_key) for candidate_key in candidate_keys):
