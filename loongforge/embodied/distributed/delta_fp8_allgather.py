...
class _GroupState:
    ...
    def __init__(
        ...
        if self.fsdp_params:
            self.num_blocks = sum((numel + block - 1) // block for numel in self.param_shard_numels)
        else:
            self.num_blocks = (shard_numel + block - 1) // block
        self.reference = torch.empty(shard_numel * world_size, dtype=torch.bfloat16, device=device)
        if self.fsdp_params:
...
def _as_contiguous_flat_input(all_gather_inputs, shard_numel):
    ...
    if expected_offset - first.storage_offset() != shard_numel:
        return None
    try:
        return first.as_strided((shard_numel,), (1,), storage_offset=first.storage_offset())
    except RuntimeError:
        return None
...
def _get_shard_input(state, all_gather_inputs, inp_split_sizes, device):
    ...
    if state.shard_buffer is None:
        state.shard_buffer = torch.empty(state.shard_numel, dtype=torch.bfloat16, device=device)
        logger.info(
...
def _delta_foreach_all_gather_copy_out(all_gather_result, fsdp_params, group):
    ...
    if state is None or not state.fsdp_params:
        return _ORIGINAL_FOREACH_ALL_GATHER_COPY_OUT(all_gather_result, fsdp_params, group)
...
def _delta_init_unsharded_param(fsdp_param):
    ...
    if was_initialized or not getattr(fsdp_param, "_delta_fp8_persistent_reference", False):
        return result
    ...
    if fsdp_param._unsharded_dtensor_spec is not None:
        unsharded_tensor = _from_local_no_grad(unsharded_tensor, fsdp_param._unsharded_dtensor_spec)
...
def _validate_config(block: int, prime_steps: int, reprime_interval: int) -> None:
    ...
    if reprime_interval < 0:
        raise ValueError(f"fsdp_delta_fp8_reprime_interval must be >= 0, got {reprime_interval}")
...
def register_delta_fp8_allgather(
    ...
    *,
    ...
    if not param_groups:
        raise RuntimeError("Cannot enable Delta-FP8 AllGather: model has no FSDP parameter groups")
