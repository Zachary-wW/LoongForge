...
def apply_peft_adapter_filter_to_state_dict(
    state_dict: dict[str], peft_class: Any
) -> dict[str, Any]:
...
def apply_peft_transformation(peft, base_model: list[MegatronModule]) -> list[MegatronModule]:
    ...
    # Log PEFT statistics
    model_to_analyze = (
        transformed_model[0] if isinstance(transformed_model, list) else transformed_model
    )
    total_params = 0
    trainable_params = 0
        ...
        if isinstance(m, TELayerNormColumnParallelLinear):
            ...
            if hasattr(m, "config") and m.config.sequence_parallel and not ub_overlap_ag:
                ...
                if te_version >= packaging.version.Version("1.5.0dev") and (
                    ...
                    # TE 1.5 introduces the option `return_layernorm_output_gathered`, so the all gather  # noqa: E501
...
class ParallelLinearAdapter(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # In some flows (e.g., after import), perform_initialization may be False to skip heavy init.  # noqa: E501
    ...
    def sharded_state_dict(
        ...
        if mamba_dim_info is not None:
            ...
            # After TP sharding: d_inner_local_tp * 2 + 2 * ngroups_local_tp * d_state + nheads_local_tp  # noqa: E501
            for k, v in linear_out_sd.items():
