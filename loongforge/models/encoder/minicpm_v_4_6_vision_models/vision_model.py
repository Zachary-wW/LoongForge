...
#
# Adapted from HuggingFace Transformers MiniCPM-V-4.6 modeling code.
...
class MiniCPMV46VisionEmbeddings(nn.Module):
    ...
    def _packed_to_embeddings(self, pixel_values: torch.Tensor, target_sizes: torch.Tensor) -> Optional[torch.Tensor]:
        if pixel_values.dim() != 4 or pixel_values.shape[0] != 1 or pixel_values.shape[2] != self.patch_size:
            return None
        ...
        for height, width in target_sizes.tolist():
            height, width = int(height), int(width)
            patch_cols = height * width
            patch = pixel_values[0, :, :, offset : offset + patch_cols * self.patch_size]
            offset += patch_cols * self.patch_size
            patch = (
                patch.reshape(3, self.patch_size, patch_cols, self.patch_size)
                .reshape(3, self.patch_size, height, width, self.patch_size)
                .permute(2, 3, 0, 1, 4)
                .reshape(patch_cols, -1)
            )
            patches.append(patch)

        if offset != pixel_values.shape[-1]:
            raise ValueError(
                f"MiniCPM packed pixel_values length {pixel_values.shape[-1]} "
                f"does not match target_sizes total width {offset}."
            )
        patch_rows = torch.cat(patches, dim=0).to(dtype=self.patch_embedding.weight.dtype)
        weight = self.patch_embedding.weight.reshape(self.embed_dim, -1)
        embeddings = F.linear(patch_rows, weight, self.patch_embedding.bias)
        return embeddings.unsqueeze(0)

    def forward(self, pixel_values: torch.Tensor, target_sizes: torch.Tensor) -> torch.Tensor:
        embeddings = self._packed_to_embeddings(pixel_values, target_sizes)
