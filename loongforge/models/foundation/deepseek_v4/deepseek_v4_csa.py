...
def _quantize_rowwise_chunked(quantizer, tensor):
    ...
    assert tensor.shape[-1] == 128, (
        f"chunked rowwise quantize assumes a 128-element last dim (one scaling block per row), got {tensor.shape[-1]}"  # noqa: E501
    )
...
class Compressor(MegatronModule):
    ...
    def _forward_thd_cp(
        ...
        """
        assert compressed_group_ids is not None, (
...
class CSAIndexerKernelFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        if packed_seq_params is not None:
            ...
            pos_in_seg = (
                torch.arange(sq, device=device, dtype=torch.int) - cu_seqlens_q[q_seg_ids].int()
            )
        ...
        if self.use_fused_indexer:
            ...
            if use_sp:
                ...
                if half % 128 != 0 or (sq_full - half) % 128 != 0:
                    index_scores, topk_indices = self.indexer_kernel(
                        q, k, weights, effective_topk, 0, None
                    )
                else:
                    s1, t1 = self.indexer_kernel(
                        q[:half], k, weights[:half], effective_topk, 0, None
                    )
                    s2, t2 = self.indexer_kernel(
                        q[half:], k, weights[half:], effective_topk, half, None
                    )
    ...
    * ``ratio == 128``: window + 128x compressed, attend to all (compressor built only)
    ...
    def __init__(
        ...
        if not self.force_unfused_dsa:
            try:
                from loongforge.models.common.experimental_attention_variant.dsa_fused_kernels import (  # noqa: E501
        ...
        if self.force_unfused_dsa:
            ...
            if self.compress_ratio > 1 and n_compressed > 0:
                ...
                if (
                    ...
                    chunk_row_valid = row_valid[
                        chunk_start:chunk_end
                    ]  # [chunk_sq] (may be short under SP-pad)
                    ...
                    with torch.no_grad():
                        # p_out: [chunk_sq, h_q, p_out_topk] -> main_attn_probs: [chunk_sq, p_out_topk]  # noqa: E501
                        ...
                        if next_pow2 != p_out_topk:
                            p_out_for_dist = F.pad(
                                p_out, (0, next_pow2 - p_out_topk), value=float("-inf")
                            )
                    ...
                    if _csa_indexer_pad > 0:  # SP padding fix: align sq dim with chunk_row_valid
                        chunk_index_scores = F.pad(
                            chunk_index_scores, (0, 0, 0, _csa_indexer_pad), value=0.0
                        )
            ...
            output = output[
                :, :sq, :
            ].contiguous()  # SP padding fix: trim padded seq dim before reshape
        ...
        if self.use_mcore_fused:
            ...
            if indexer_loss_coeff > 0:
                ...
                with torch.no_grad():
                    ...
                    main_attn_probs = main_attn_probs[:, :p_out_topk]
                    main_attn_probs = torch.nan_to_num(
                        main_attn_probs, nan=0.0, posinf=0.0, neginf=0.0
                    )
                    main_attn_probs = main_attn_probs.masked_fill(
                        ~chunk_row_valid.unsqueeze(-1), 0.0
                    )
                ...
                if chunk_sq > index_scores_topk.size(0):
                    index_scores_topk = F.pad(
                        index_scores_topk,
                        (0, 0, 0, chunk_sq - index_scores_topk.size(0)),
                        value=0.0,
                    )
                ...
                index_attn_probs = torch.nan_to_num(
                    index_attn_probs, nan=0.0, posinf=0.0, neginf=0.0
                )
        ...
        if use_indexer_loss:
            if not getattr(self.config, "dsa_indexer_use_sparse_loss", True):
                raise NotImplementedError(
                    "DSv4 THD CP indexer loss only implements the sparse variant; set dsa_indexer_use_sparse_loss=True."  # noqa: E501
                )
            h_q = query.size(1)
