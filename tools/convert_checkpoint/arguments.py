...
def parse_args(title=None):
    ...
    if _GLOBAL_ARGS is not None:
        return _GLOBAL_ARGS
    """Parse all arguments."""
    parser = argparse.ArgumentParser(description="LoongForge-Tool Arguments", allow_abbrev=False)
...
def _add_checkpoint_args(parser):
    ...
    group.add_argument("--load_platform", type=str, default=None, choices=["huggingface", "mcore"])
    group.add_argument("--save_platform", type=str, default=None, choices=["huggingface", "mcore"])
    ...
    group.add_argument(
        ...
        action="store_true",
        help=(
            "If enabled, in converting FP8 checkpoint, skip the `dequantize + re-quantize`, "
    ...
    group.add_argument(
        ...
        default=0.0,
        help=(
            "Epsilon value added to the amax calculation to avoid divised by zero "
    ...
    group.add_argument(
        "--hf-dequantize-int4",
        "--hf_dequantize_int4",
        dest="hf_dequantize_int4",
        action="store_true",
        help=(
            "When loading a HuggingFace checkpoint, materialize compressed-tensors "
    ...
    group.add_argument(
        "--hf-dequantize-mxfp4",
        "--hf_dequantize_mxfp4",
        dest="hf_dequantize_mxfp4",
        action="store_true",
        help=(
            "When loading a HuggingFace checkpoint, materialize MXFP4 packed "
    ...
    group.add_argument(
        ...
        dest="hf_quant_config_file",
        type=str,
        default=None,
        help="Optional HF config.json path for compressed-tensors quantization_config.",
    )
    group.add_argument(
        "--hf-official-config-file",
        "--hf_official_config_file",
        dest="hf_official_config_file",
        type=str,
        default=None,
        help=(
            "Official HuggingFace config.json used when saving HF checkpoints. "
            "For Kimi K2.5 this drives compressed-tensors INT4 output format."
        ),
    )
    group.add_argument(
        "--hf-pack-quantized-from-config",
        "--hf_pack_quantized_from_config",
        dest="hf_pack_quantized_from_config",
        action="store_true",
        help=(
            "When saving a HuggingFace checkpoint, pack matching .weight tensors "
            "with compressed-tensors according to --hf-official-config-file or "
            "--hf-quant-config-file."
        ),
    )
    group.add_argument(
        "--hf-pack-quantized-target-regex",
        "--hf_pack_quantized_target_regex",
        dest="hf_pack_quantized_target_regex",
        type=str,
        default=None,
        help=(
            "Regex over HF module names to select weights for pack-quantized "
            'compressed-tensors output. The suffix ".weight" is not included.'
        ),
    )

    group.add_argument(
        "--fp8_quant_transfer_type",
        type=str,
        default="float32",
    ...
    group.add_argument(
        "--hf_checkpoint_device",
        type=str,
        default="cpu",
        help="Device used to load and save HF checkpoint, default is cpu",
    )
...
def _add_common_args(parser):
    ...
    group.add_argument(
        ...
        action="store_true",
        help=(
            "When virtual pipeline is enabled, VIT is only in the first virtual stage,"
...
def _add_megatron_args(parser):
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Note: This argument will be converted into --custom_pipeline_layers."
