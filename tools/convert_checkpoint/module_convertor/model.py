...
class Model:
    ...
    def get_visual_args(args):
        ...
        visual_args.tensor_model_parallel_size = (
            args.encoder_tensor_model_parallel_size
            if args.encoder_tensor_model_parallel_size is not None
    ...
    def convert_from_common(self, platform, target_c_config, layer_dict, expert_dict=None, target_c_vision_config=None):
        ...
        if platform == "mcore":
            ...
            if no_encoder:
                ...
                McoreCheckpoint.convert_from_common_vlm(
                    m_ckpt,
                    m_vision_ckpt,
                    self.c_vision_patch_config,
                    self.c_ckpt,
                    self.c_vision_ckpt,
                    target_c_config,
                    target_c_vision_config,
                    args.save_ckpt_path,
                    layer_dict,
                    expert_dict,
                )
        ...
        if platform == "huggingface":
            ...
            if p > 0 or self.c_vision_patch_config is None:
                hf_ckpt.convert_from_common(
                    self.c_ckpt, layer_dict, expert_dict=expert_dict, save_path=args.save_ckpt_path
                )
            else:
                visual_args = Model.get_visual_args(args)
...
def _convert_checkpoint(args):
    ...
    def convert_one_p(p, cur_ep_ids=None):
        ...
        model.convert_from_common(
            ...
            target_c_config,
            layer_dict,
            expert_dict=ep_expert_mapping,
            target_c_vision_config=target_c_vision_config,
        )
...
def test():
    ...
    for p in range(pp):
        v3_params[p] = {}
        for e in range(ep):
            v3_params[p][e] = torch.load(
                f"/mnt/cluster/deepseek-ai/DeepSeek_V3_tp1pp2ep4/release/mp_rank_00_{p:03d}_{e:03d}/model_optim_rng.pt"
            )
