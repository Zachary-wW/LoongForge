...
for sub_dir in sub_dirs:
    ...
    if "model" in ckpt.keys():
        ...
        if (
            args.pipeline_model_parallel_size == 1 or int(splits[3]) == 0
        ) and "foundation_model.embedding.word_embeddings.weight" in ckpt["model"]:
            ckpt["model"]["encoder_model.text_encoder.word_embeddings.weight"] = ckpt["model"][
        ...
        if (
            args.pipeline_model_parallel_size == 1 or int(splits[3]) == 0
        ) and "foundation_model.embedding.word_embeddings.weight" in ckpt["model0"]:
            ckpt["model0"]["encoder_model.text_encoder.word_embeddings.weight"] = ckpt["model0"][
                "foundation_model.embedding.word_embeddings.weight"
            ]
