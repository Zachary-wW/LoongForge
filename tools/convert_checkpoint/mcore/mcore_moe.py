...
class McoreMoe(McoreBase):
    ...
    def get_moe_path_info(
        ...
        if self.args.moe_grouped_gemm:
            m_name_prefix = (
                self.name_map[MOE_GROUPED_GEMM_EXPERT]
    ...
    def common_e_to_mcore(
        ...
        path_info = self.get_moe_path_info(
            expert_name, name, layer_id, m_layer_id, expert_id, layer_prefix, name_prefix
        )
        ...
        if self.etp is None:
            ...
            self.update_mcore_expert_weight(
                ...
                m_tp=self.tp,
                has_extra=has_extra,
                mcore_extra_path=mcore_extra_path,
                bias_list=bias_list,
            )
        else:
            weight_list, bias_list = self.get_chunked_weight(
    ...
    def get_mcore_e_weight_list(self, e_m_dict, t_name, mcore_weight_path, mcore_bias_path):
        ...
        for mt in range(m_tp):
            ...
            weight_list[mt], bias_list[mt], weight_scale_list[mt] = self.get_weight_by_tp(
                e_m_dict[mt], t_name, mcore_weight_path, mcore_bias_path
            )
