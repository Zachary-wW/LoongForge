    ...
    for module in [vision_model, adapter, patch]:
        ...
        for etp_rank in range(etp_size):
            ...
            for dtp_rank in range(dtp_start, dtp_end, step):
                merge_dict(module[etp_rank]["model"], language_model[dtp_rank]["model"])

    for dtp_rank in range(dtp_size):
        language_model[dtp_rank]["model"]["encoder_model.text_encoder.word_embeddings.weight"] = language_model[
            dtp_rank
        ]["model"]["foundation_model.embedding.word_embeddings.weight"]
        ...
        for etp_rank in range(etp_size):
            ...
            for dtp_rank in range(dtp_start, dtp_end, step):
                if "model" in language_model[0][dtp_rank].keys():
                    merge_dict(module[etp_rank]["model"], language_model[0][dtp_rank]["model"])
                else:
                    assert "model0" in language_model[0][dtp_rank].keys()
                    merge_dict(module[etp_rank]["model"], language_model[0][dtp_rank]["model0"])
