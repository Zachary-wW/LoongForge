...
def _validate_source_dataset(
    ...
    if not codebase_version.startswith("v2"):
        raise ValueError(
            f"DreamZero preparation currently requires LeRobot v2, got codebase_version={codebase_version!r}"  # noqa: E501
        )
    if not info.get("fps"):
...
def _build_modality_metadata(info: dict, preset: DreamZeroDatasetPreset) -> dict:
    return {
        ...
        "annotation": {
            name: {"original_key": original_key} for name, original_key in preset.annotation
        },
    }
...
def _compute_relative_statistics(
    ...
    for parquet_path in tqdm(parquet_paths, desc="Collecting relative-action statistics"):
        ...
        for key in preset.relative_action_keys:
            ...
            if state.shape[1] != action.shape[1]:
                raise ValueError(
                    f"Relative-action dimensions differ for {key}: state={state.shape[1]}, action={action.shape[1]}"  # noqa: E501
                )
    ...
    return {
        key: _summarize(np.concatenate(values, axis=0)) for key, values in relative_values.items()
    }
