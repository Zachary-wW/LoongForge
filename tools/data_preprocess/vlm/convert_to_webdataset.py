...
def _read_media(directory, reference, index, vision):
    ...
    if reference.startswith("data:"):
        ...
        try:
            ...
            raise ValueError(f"Invalid base64 {vision} data URI at sample {index}") from exc
...
def convert_to_wds(args):
    ...
    with wds.ShardWriter(tar, maxcount=args.maxcount, maxsize=args.maxsize) as shard_writer:
        for index, entry in enumerate(tqdm(data)):
            if args.sample_type == "vqa" or args.sample_type == "caption":
                ...
                default_key = f"image_{index}" if image_path.startswith("data:") else image_path
                sample = {
    ...
    write_config(EPath(args.output_dir), args.media, args.sample_type)
...
def write_config(path: EPath, media: str = None, sample_type: bool = False):
    ...
    if sample_type == "vqa":
        ...
        dataset_definition = {
            "sample_type": {"__module__": "megatron.energon", "__class__": "CaptioningSample"},
            "field_map": {"image": "jpg", "caption": "json[captions][0][content]"},
        }
    else:
