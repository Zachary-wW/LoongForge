...
class HashBucketProcessor:
    ...
    DTYPE_SAMPLE_INFO = np.dtype(
        [
            ...
            (
                HASH_BUCKET_SAMPLE_ID_KEY,
                object,
            ),  # Store sample ids as Python strings; fixed-width Unicode bloats pickle files.
        ]
    )
    ...
    def get_hash_buckets_summary(self) -> dict:
        ...
        items_by_size = {"large": 0, "medium": 0, "small": 0}
    ...
    def pack_best_fit_decreasing(self, box_capacity: int = 16384) -> List[np.ndarray]:
        ...
        output_boxes = [self._pack_items_to_array(box) for box in boxes if box]
    ...
    def pack_large_seed_parallel_multithread(
        ...
        class SharedResourceManager:
            def __init__(self, hash_buckets, small_keys, large_keys):
                ...
                self.available_small_keys = sorted(
                    [k for k in small_keys if k in hash_buckets and len(hash_buckets[k]) > 0]
                )
            ...
            def get_seed_item(self, seed_key: int) -> tuple:
                ...
                with self.lock:
                    if seed_key in self.hash_buckets and len(self.hash_buckets[seed_key]) > 0:
            ...
            def get_item_by_key(self, target_key: int) -> tuple:
                ...
                with self.lock:
                    if target_key in self.hash_buckets and len(self.hash_buckets[target_key]) > 0:
            ...
            def get_stats(self) -> dict:
                ...
                with self.lock:
                    small_items_count = sum(
                        len(self.hash_buckets[k]) for k in self.small_keys if k in self.hash_buckets
                    )
                    large_items_count = sum(
                        len(self.hash_buckets[k]) for k in self.large_keys if k in self.hash_buckets
                    )
    ...
    def pack_with_min_items_constraint_multithread(
        ...
        if max_workers is None:
            max_workers = min(
                os.cpu_count(), 6
            )  # Constrained problem is computationally intensive, reduce thread count
        ...
        def pack_single_seed_with_constraints(
            seed_key: int, shared_manager: EnhancedSharedManager, thread_id: int
        ) -> Tuple:
            ...
            try:
                ...
                while (
                    ...
                    and items_count < min_items * 8  # Allow exceeding minimum count
                    ...
                    # Quick feasibility check
                    if items_count < min_items and not is_feasible_quick_check(
                        remaining_capacity, items_count, available_keys, min_items
                    ):
                    ...
                    # Select next key
                    target_key = select_optimal_key(
                        strategy, available_keys, remaining_capacity, items_count, min_items
                    )
                ...
                if success:
                    ...
                    return (
                        ...
                        current_capacity,
                        items_count,
                        f"Success: {items_count} items, load ratio {load_ratio:.1%}",
                    )
                else:
    ...
    def pack_with_flexible_seeds(
        ...
        if seed_strategy == "auto":
            ...
            large_keys = [k for k in self.hash_buckets.keys() if min_size <= k <= max_size]
            ...
            try:
                ...
                if success:
                    ...
                    return (
                        ...
                        current_capacity,
                        items_count,
                        f"Success: {items_count} items, load rate {load_ratio:.1%}",
                    )
                else:
                    # Packing failed, rollback
                    shared_manager.rollback_items(used_items)
    ...
    def check_hash_buckets_state(self):
        ...
        # total_keys = len(self.hash_buckets)
        total_keys = len(
            [key for key in self.hash_buckets if len(self.hash_buckets[key]) > 0]
        )  # Do not delete keys with zero elements
